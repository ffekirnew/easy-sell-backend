...
class ProductRepository(ABCProductRepository):
    ...
    def add(self, product: Product) -> ObjectId:
        product_id = self.products.insert_one(
            ProductModel.from_entity(product).to_dict()
        ).inserted_id
        return product_id
    ...
    def update(self, product_id: ObjectId, product: Product) -> bool:
        status = self.products.update_one(
            {"_id": product_id}, {"$": ProductModel.from_entity(product).to_dict()}
        )
    ...
    def get_user_products(self, user_id: str) -> List[Product]:
        return list(map(Product.from_dict, self.products.find({"user_id": user_id})))
